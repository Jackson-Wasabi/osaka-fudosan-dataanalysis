...
## 2026-06-23 — Phase 2 / B-1 README.md 作成

- 検証済み(final_validation.md)の数値のみでREADME.md作成。構成: サマリー＋想定ユーザー＋免責→成果(ファネル/Top5)→使用データ→技術スタック→フロー図→設計4判断(Q1-Q4)→モデル評価表→スコア設計→再現手順→ディレクトリ→限界→今後→リンク。採用担当が上から3分で読める順。
- 3枚PDFをリポジトリ外(親フォルダ)から docs/osaka_condo_analysis_summary_3pages.pdf にコピー同梱(英語名でリンク切れ防止・494KB)。READMEリンク2か所を実ファイル名に更新。
- 残プレースホルダ3点(ユーザー記入待ち): Tableau Public URL×2・メールアドレス×1。Tableau URLはPhase3公開後。
- B-1完了。次は **feature/step10-model → main マージ → GitHub push(gh未導入→gh導入 or PAT)**。その前に未コミットのdocs(work_log/decision_log/final_validation/README/手順書/スクリプト修正)をコミット。

## 2026-06-23 — Phase 2 / B-2 GitHub公開前の秘匿情報点検

- 追跡ファイル115件を点検。**raw成約CSV・GIS raw・profiles.yml・.env・認証情報は一切追跡なし**（.gitkeepのみ）。.gitignoreは`data/raw/*`形式で機能。公開対象のdata配下=gis/processed(L01地価/N02鉄道=国土数値情報の公開オープンデータ)・samples(公式匿名化フォーマット11行・氏名/建物住所なし)で安全。
- 機密文字列スキャン(git grep)で唯一の検出=reinfolib APIキー直書き（scripts/download_transactions*.py 2ファイル）。サイト配布の公開フロントエンドキーで実害低だが、採用評価上「公開リポにキー直書きしない作法」を示すため**環境変数化を採用**（ユーザー選択）。
- 修正: `KEY = os.environ.get("REINFOLIB_KEY", "<公開キー>")` に変更＋コメントで公開キーである旨明記。両ファイルpy_compile OK。直書きはデフォルト値のみに。
- 注意（私のverifyスクリプトのバグ記録）: `git ls-files --error-unmatch` のエラーメッセージを`grep -q .`で拾うと未追跡でも誤検出。判定は終了コードで行う。
- B-2完了。次は **B-1 README.md 叩き台作成** → ブランチ feature/step10-model → main マージ → push（gh未導入）。

## 2026-06-23 — Phase 1 分析の最終検証（読み取り専用・docs/final_validation.md 作成）

- ポートフォリオ完成手順書（docs/portfolio_completion_manual.html/.pdf）を作成。ChatGPT案を土台に、このプロジェクト特有の誤り2点を訂正（①学習2021-24/検証2025←案の2019-20は誤り、②新規大冊PDFは作らずREADME集約、notebooks不要）。7フェーズ構成・未経験者向け再現手順。
- Phase 1（最優先）として、PDF/Tableau掲載値を実データで全点検（読み取り専用スクリプト＋アドホッククエリ。ADCコピー＋env でgcloudハング回避）。**全項目一致**。
  - ファネル: 2,106件 / 151駅(model_eligible) / 66駅(10件以上) / 本命10駅 ✅
  - 学習/検証: train 2021-24（各年1,258/1,448/1,495/1,605）・test 2025（2,106）をsplit列で直接確認＝**リークなし**を実証 ✅
  - モデル評価: Baseline MAE186,117/hit0.176・**Model C 120,567/0.269（採用・Baseline比約35%改善）**・Model F 101,093/0.329 すべてPDF①と一致 ✅
  - スコア: バランス型(priority_score)堺筋本町80.8…、割安重視型(score_discount)平野85.6/玉造84.1/大国町82.4/堺筋本町81.9/伝法79.6/森ノ宮78.8… PDF3ページと完全一致。PDF1ページTop5は割安重視ランク2-6（平野は逆選択でhigh_risk0.857→本命外）で論理整合 ✅
  - リーク3点（駅中央値=訓練期間のみ/公示地価=採用Model C未使用・Dは不採用/補完中央値=訓練期間のみ）確認 ✅
- 注意メモ: 2,106(総取引)と2,097(model_eligible・モデル評価n_test)の差は仕様で矛盾でない、と final_validation.md 6章に明記。
- スコア内訳列(sc_value/sc_reliability/sc_land/sc_quality/risk_penalty)がmartに存在＝A-1(内訳可視化)の素材あり。
- 成果物: docs/final_validation.md。**Phase 1完了 → 次は Phase 2 GitHub整備（README作成・秘匿情報点検・main統合）**。

## 2026-06-23 — 設計レビュー＋②区別バイアス図のダンベル差し替え検討（読み取り専用検証）

- 3枚構成PDF（大阪市中古マンション_分析資料）をレビュー。スライド②「C vs F 各区の割安%」の差し替えを検討。
- model_bias_by_ward を読み取り専用クエリで実測確認（C降順、20区）。
  - **F−C ＝ +11.8〜+18.2%（平均+14.5%）のほぼ一律の右シフト**を確認（D-033の記録と整合）。
  - 中心区: C過小予測（北区−18.6%・中央区−14.8%）→ F が≈0に是正（北区−6.8%・中央区−2.4%）＝Fの精度改善の正体はここだけ。
  - 周縁区: C過大予測（住吉+20.5%・平野+15.1%）→ F が+38.7%/+32.7%に膨張＝偽の割安を悪化。
  - レンジ: C[−18.6%,+20.5%]幅0.39 → F[−6.8%,+38.7%]幅0.455。Fはバラつきを直さず微増。
- **自己訂正**: 先のレビューで②を「結論と矛盾」と評したが実データ上は矛盾なし（周縁区ではFの棒が長く主張どおり）。問題は「一律シフトの機構が読み取りにくい」点に修正（重大→中程度）。
- 差し替え方針: signed_bias を ward×model のダンベル（C橙→F青を灰線で連結、ward は平均bias降順）で「一律シフト＝精度改善≠ランキング改善」を直接可視化。分散バー案は定数シフトで差が出ず不採用。
- 次: Tableauで②をダンベルに差し替え→注釈（実数字版）反映→デッキ更新。
